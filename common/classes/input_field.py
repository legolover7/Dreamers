...
class InputField():
    def __init__(self, rect, font, default_text="Enter text", increases_upwards=False, title="", title_font=None):
    ...
    def draw(self, active, offset=0):
        ...
        if self.title != "":
            ...
            Globals.VID_BUFFER.blit(self.title_font.render(self.title, True, Colors.white), (self.x, self.y - text_height - 4 + offset))
        ...
        # Display background
        pyg.draw.rect(Globals.VID_BUFFER, Colors.gray, (self.x, self.y - (extra_height if self.increases_upwards else 0) + offset, self.width, self.height + extra_height), border_radius=5)
        pyg.draw.rect(Globals.VID_BUFFER, Colors.dark_gray, (self.x+2, self.y+2 - (extra_height if self.increases_upwards else 0) + offset, self.width-4, self.height-4 + extra_height), border_radius=5)
        ...
        if len(text_lines) == 0 and (Globals.cursor_frame > Globals.cursor_timeout or (Globals.cursor_frame % Globals.cursor_period < Globals.cursor_period / 2)):
            ...
            pyg.draw.rect(Globals.VID_BUFFER, Colors.white, (self.x + 4, self.y + vertical_offset + (self.height - text_height)/2 + offset, 2, text_height))
